...
The portal reads the same `Users` table so role changes have one source of truth. Give the portal
application pool read access to the tracker database:

```sql
CREATE LOGIN [IIS APPPOOL\SonAeroPortal] FROM WINDOWS;

USE [ProjectTracker];
CREATE USER [IIS APPPOOL\SonAeroPortal] FOR LOGIN [IIS APPPOOL\SonAeroPortal];
ALTER ROLE db_datareader ADD MEMBER [IIS APPPOOL\SonAeroPortal];
```
...
Copy `apps\project-tracker\deployment\backup-project-tracker.sql` to the server, then schedule it with SQL Server Agent if available, or Windows Task Scheduler plus `sqlcmd`.
