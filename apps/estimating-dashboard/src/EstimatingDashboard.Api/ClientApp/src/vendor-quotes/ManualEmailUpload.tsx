...
export default function ManualEmailUpload({ detail, selectedThread, disabled, resetToken, onDirty, onImported, compact = false }: {
  ...
  return <div className={`qs-email-upload${compact ? ' is-compact' : ''}`}>
    <div className={`qs-email-drop${dragging ? ' is-dragging' : ''}`} role="group" aria-label="Attach an email" onDragEnter={event => { event.preventDefault(); dragDepth.current++; setDragging(true) }} onDragOver={event => { event.preventDefault(); event.dataTransfer.dropEffect = disabled ? 'none' : 'copy' }} onDragLeave={event => { event.preventDefault(); dragDepth.current = Math.max(0, dragDepth.current - 1); if (!dragDepth.current) setDragging(false) }} onDrop={event => { event.preventDefault(); event.stopPropagation(); dragDepth.current = 0; setDragging(false); choose(event.dataTransfer.files) }}>
      <span className="qs-email-drop-icon">{dragging ? <Upload size={19} /> : <Mail size={19} />}</span><div><strong>{dragging ? 'Drop your saved email here' : compact ? 'Drop a saved email' : 'Keep an email with this record'}</strong><p>{compact ? '.msg or .eml · up to 30 MB' : 'Drop a saved .msg or .eml file here, or choose a file.'}</p></div><button type="button" className="vq-button" disabled={disabled} onClick={() => input.current?.click()}><Paperclip size={15} />Attach email</button>
    ...
    {!compact && <p className="qs-email-drop-hint">If dragging from Outlook does not work, save the message as .msg first. One email at a time, up to 30 MB.</p>}
