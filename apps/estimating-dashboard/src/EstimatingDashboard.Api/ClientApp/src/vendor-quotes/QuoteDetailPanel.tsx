import { ArrowLeft, ArrowUpRight, CalendarDays, ChevronRight, GitBranch, History, Mail, Pencil, Plus } from 'lucide-react'
...
export default function QuoteDetailPanel({ detail, statuses, threadStatuses, canManage, draftReset, focused, onBack, onChanged, onDirty, guard }: {
  ...
  const [tab, setTab] = useState<'activity' | 'threads' | 'emails'>('activity')
  ...
  async function saveOverview() {
    if (!overviewDirty || overviewSaving || saving) return
  ...
  return <section className="vq-detail" aria-label={`Quote ${quote.quoteNumber} details`}>
    ...
    <div className="vq-detail-body">
      {tab !== 'threads' && <div className="vq-scope"><button onClick={() => transition(() => setThreadId(null))} disabled={!thread}>Quote {quote.quoteNumber}</button>{thread ? <><ChevronRight size={14} /><span>{scopedLabel}</span></> : <span className="vq-scope-hint">All parts, threads, and updates</span>}</div>}
