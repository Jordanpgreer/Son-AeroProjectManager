  ...
  return <>
    ...
    <section className="dashboard-results">
      {loading ? <section className="panel skeleton-panel"><div className="skeleton-line lg"/><div className="skeleton-line"/><div className="skeleton-line" style={{ width: '72%' }}/></section> :
        grouped.length ? grouped.map(group => <article key={group.category.id} className="panel results-group"><div className="panel-head compact"><div className="panel-head-text"><span className="eyebrow">{group.category.title}</span><h2>{group.results.length} result{group.results.length === 1 ? '' : 's'}</h2></div></div><div className="results-list">{group.results.map(item => <button key={item.id} type="button" className={`result-card ${item.drawingId ? 'clickable' : ''}`} disabled={!item.drawingId} onClick={() => item.drawingId && onOpenDrawing(item.drawingId)}><div className="result-head"><div><strong>{item.title}</strong><span className="result-id">{item.identifier}</span></div><span className="result-category">{item.categoryLabel}</span></div><p className="result-subtitle">{item.subtitle}</p><dl className="result-meta">{item.customer && <div><dt>Customer</dt><dd>{item.customer}</dd></div>}{item.specificationNumber && <div><dt>Spec</dt><dd>{item.specificationNumber}</dd></div>}{item.workOrder && <div><dt>Work order</dt><dd>{item.workOrder}</dd></div>}{item.reportNumber && <div><dt>Report</dt><dd>{item.reportNumber}</dd></div>}</dl><div className="token-list">{item.tags.map(tag => <span key={tag} className="token-chip">{tag}</span>)}</div><p className="result-note">{item.note}</p></button>)}</div></article>) :
