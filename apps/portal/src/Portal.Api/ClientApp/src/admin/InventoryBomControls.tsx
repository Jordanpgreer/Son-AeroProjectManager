...
export default function InventoryBomControls({ sheet, source, columns, change, customize, addFields, filters, sorted }: {
  ...
  return <section className="ac-bom-setup" aria-label="Inventory BOM setup">
    ...
    <details className="ac-format-options"><summary>Report Notes And Limits</summary><p>Non-archived / latest revision are item filters, not BOM approval status. Times use hours:minutes:seconds. Qty Req is populated only for requires-basis quantities. Full inventory reports can take several minutes. Limits: 5,000 items, 100,000 rows, 10 minutes and 20 MB preview data. A failed or limited pull never produces a partial workbook. This is not a validated ERP import.</p></details>
  </section>
}
