...
import { AlertTriangle, CheckCircle2, Eye, MessageCircleQuestion, Save, ShieldCheck, Sparkles } from 'lucide-react'
...
/**
 * Every Benny setting lives here, on its own admin page. The page is reachable
 * only by full Arda administrators - the same gate the ManageWalkthrough policy
 * enforces on the API, so module admins cannot see it or save through it.
 *
 * The walkthrough (training) toggle shares this settings record but belongs to
 * Project Tracker onboarding, so it is read here and written back untouched.
 */
...
type BennyDraft = Pick<
  WalkthroughSettings,
  'assistantEnabled' | 'assistantName' | 'assistantIdleModules' | 'assistantIdleDelayMinutes'
>
...
const DEFAULT_DRAFT: BennyDraft = {
...
function toDraft(value: WalkthroughSettings): BennyDraft {
  return {
...
export default function BennySettingsPanel() {
  const [settings, setSettings] = useState<WalkthroughSettings | null>(null)
  const [draft, setDraft] = useState<BennyDraft>(DEFAULT_DRAFT)
  ...
  async function save() {
    ...
    try {
      const next = await trackerApi<WalkthroughSettings>('/api/settings/walkthrough', {
        method: 'PUT',
        body: JSON.stringify({
          // Owned by the Project Tracker onboarding page; echoed back unchanged.
      ...
      setMessage('Benny settings saved.')
    } catch (cause) {
  ...
  function toggleIdleModule(moduleKey: string, enabled: boolean) {
    setDraft((current) => ({
      ...
      assistantIdleModules: enabled
        ? BENNY_MODULES.map((module) => module.key)
          .filter((key) => key === moduleKey || current.assistantIdleModules.includes(key))
  ...
  return (
    <section className="admin-surface" aria-labelledby="benny-heading">
      <header className="admin-surface-head">
        <div>
          <span className="kicker">Guided help</span>
          <h2 id="benny-heading">Benny</h2>
          <p>Every setting for the deterministic helper and his cosmetic idle activity, across all modules.</p>
        </div>
      ...
      {!settings && !error ? <div className="admin-loading" role="status">Loading Benny settings…</div> : settings ? (
        <>
          ...
          <div className="admin-inline-save">
            <p><ShieldCheck size={15} /> Benny is a help feature only; these settings do not change group permissions.</p>
            <button className="solid-button" type="button" disabled={!changed || Boolean(nameError) || Boolean(idleDelayError) || saving} onClick={() => void save()}>
              <Save size={15} /> {saving ? 'Saving…' : 'Save Benny settings'}
            </button>
          </div>
        </>
      ) : null}
