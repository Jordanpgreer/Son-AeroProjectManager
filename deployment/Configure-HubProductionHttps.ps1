...
function Replace-ProtectedProductionStateFile {
    ...
    if ((Split-Path -Parent $TemporaryPath) -ine $directory) {
        throw 'Production state replacement source and destination must be siblings.'
    }
    ...
    Assert-NoReparsePathChain -Path $TemporaryPath
    Assert-NoReparsePathChain -Path $DestinationPath
    Assert-NoReparsePathChain -Path $backupPath
    $backupExists = $false
    ...
    Assert-ProtectedPath -Path $TemporaryPath
    Assert-ProductionStateProtection -Path $DestinationPath
    ...
    try {
        ...
        if ($backupItem.PSIsContainer -or
            ...
            throw "Production state replacement backup '$backupPath' is not a regular file."
        }
        Assert-ProtectedPath -Path $backupPath
        Assert-ProductionStateProtection -Path $DestinationPath
        ...
        if ($committedHash -cne $temporaryHash -or $preservedHash -cne $priorDestinationHash) {
            throw 'Committed or preserved production state bytes do not match the pre-replacement hashes.'
        }
        $backupSafeToDelete = $true
    }
    catch {
        if ($replaceCompleted) {
            throw "Production state replacement committed but verification failed; prior protected state is preserved at '$backupPath'. $($_.Exception.Message)"
        }
        throw
    }
    finally {
        ...
        if ($backupSafeToDelete) {
            try {
                ...
                if ($backupItem.PSIsContainer -or
                    ($backupItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) {
                    throw "Production state replacement backup '$backupPath' became unsafe before cleanup."
                }
                Assert-ProtectedPath -Path $backupPath
                Remove-Item -LiteralPath $backupPath -Force -ErrorAction Stop
                if ([IO.File]::Exists($backupPath) -or [IO.Directory]::Exists($backupPath)) {
                    throw "Production state replacement backup '$backupPath' still exists after cleanup."
                }
            }
            catch {
                Write-Warning `
                    "Production state was committed and verified, but its protected prior-state backup remains at '$backupPath': $($_.Exception.Message)" `
...
function Write-ProductionState {
    ...
    try {
        ...
        if (Test-Path -LiteralPath $StatePath) {
            Replace-ProtectedProductionStateFile -TemporaryPath $temporary -DestinationPath $StatePath
        }
