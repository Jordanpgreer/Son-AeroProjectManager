...
- Limits: 8 sources, 80 combined output fields, 1-50,000 records per individual source/filter invocation, 100,000 fetched rows across sources, 1,000 API calls, 20 MB per response/preview, and two concurrent runs. Fulcrum pages remain limited to 500 records per API call; Arda follows documented `skip`/`take` pagination until the selected total is reached. Reports requesting over 5,000 records receive a ten-minute timeout instead of three minutes. Exceeding any limit fails rather than silently exporting partial data. Sources without complete pagination still require narrower filters.
- Inventory BOM runs have separate bounded limits because every parent item requires related routing and input-item calls: 10,000 starting items, 100,000 composed rows, 20,100 API calls, and 30 minutes, with at most three parallel item readers per report. Split larger item catalogues with item-number or revision filters. The 20 MB response/preview and two-concurrent-report limits still apply. Every selected record must finish before a snapshot/download is made available.
